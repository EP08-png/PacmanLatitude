...
var NUM_ROWS = 10;  // 14
var NUM_COLUMNS = NUM_ROWS;  // 30
var WALL_PCT = .20;   // % of walls on board
const SQUARE_SIZE = 50;  // pixel size of individual squares
var NUM_GHOSTS = 1;   // number of ghosts to create on each level
var GHOST_SMARTS = 80; // % of time ghost moves towards pacman during regular mode or away from pacman during pp mode
  // delay in seconds for dead ghosts to respawn
var SAFE_ZONE_SIZE = 4; // rows/columns of safety in upper left corner when pacman spawns or respawns
var GHOST_SPEED = 1;  // how often ghosts move in seconds
var RESET_PLAYER_DELAY = 3; // seconds
const LIVES_START = 3;
const BOMBS_START_COUNT = 10;
const POWER_PELLETS_START_COUNT = Math.min(Math.max(Math.floor(((NUM_ROWS*NUM_COLUMNS)+1)/25),1),10);  // num of pp per board, 1 per 40 squares
var POINTS_PER_GHOST = 10;
var POINTS_PER_Pellet = 1;
var POWER_PELLET_DELAY = 8; // pacman gets this many seconds to kill ghosts after eating pp
var BOMB_DELAY = 4;   // bombs blow up in this many seconds
...
const ItemsGraphics = [
  ...
  'graphics/Coffee.png',
  'graphics/Taco.png',
  'graphics/Glasses.png',
  'graphics/Grenade.png'
];
const ItemList = document.getElementById('ItemList');
const ItemInfos = [
  "Medkit: +1 Life",//0
  "Ghost Repelant: Adds 15% to ghost respawn cooldown",
  "Respawn Anchor: Decreases Player Respawn Delay by 10%",
  "Nuke: Increases bomb spawn rate",
  "Brain: Decreases Ghost Smarts by 4 points",
  "Coffee: Increases Power Pellet Effect Time by 1 Sec (Max is 4)",
  "William's Taco: Adds 1 point to all scoring types",
  "Glasses: 50% Chance to increase or decrease board size by 1",
  "Grenade: +1-7 Bombs"
...
function createBoard()
{
    ...
    // Create the squares
    for (var i=-1;i<NUM_COLUMNS*NUM_ROWS;i++)
    {
...
function drawEmptyBoard()
{
  ...
  //set all to 0 to avoid errors
  for (i=-1; i< NUM_ROWS*NUM_COLUMNS; i++)
    {
      powerPellets[i] = 0;
      goodBombs[i] = 0;
      poison[i] = 0;
  ...
  // respawn pacman
  setTimeout( respawnPacmanTimer, RESET_PLAYER_DELAY*1);
...
function drawItemBoard()
{
  ...
  //Board for Special Items
  isOnItemLevel = true;

  squares[3].innerHTML = "I";
  squares[4].innerHTML = "T";
  squares[5].innerHTML = "E";
  squares[6].innerHTML = "M";
...
function spawnSpecialItems()
{
  ...
  SpawnPositions = ItemSpawns.slice();
  for (i=0; i<10; i++){
    var type = getRandomInt(0,MaxItemIndex);
    var pos = SpawnPositions[getRandomInt(0,ItemSpawns.length-1)]
    if(pos == null)
      {
        i--;
        continue;
      }
    squares[pos].innerHTML = "<img src="+ ItemsGraphics[type]+">";
...
function PickupItem(type, pos)
{
  switch(type)
  {
    ...
    GHOST_RESPAWN_DELAY += GHOST_RESPAWN_DELAY*.1;
    Math.min(Math.max(GHOST_RESPAWN_DELAY,1), 10);
    break;

    case 2:
      //respawn anchor
      RESET_PLAYER_DELAY -= RESET_PLAYER_DELAY*0.15;
      break;

    case 3:
      //Nuke
      BOMB_DELAY--;
      Math.min(Math.max(BOMB_DELAY,3), 10);
      break;
    
    case 4:
      //Brain
      GHOST_SMARTS-=4;
      Math.min(Math.max(GHOST_SMARTS,30), 100);
      break;
    case 5:
        //Coffee
        POWER_PELLET_DELAY++;
        Math.min(Math.max(POWER_PELLET_DELAY,1), 12);
        break;
    case 6:
        //Taco
       POINTS_PER_GHOST+=1;
       POINTS_PER_Pellet+=1;
       Math.min(Math.max(POINTS_PER_Pellet,1), 3);
      break;
...
function addItemToList(type){
  var img = document.createElement('img');
    img.src = ItemsGraphics[type]; // Set the source of the image
     // Set alt text for accessibility
    img.style.width = '48px'; // Optional: Set the image width
    img.style.height = 'auto'; // Optional: Maintain aspect ratio
    img.title = ItemInfos[type];
...
function resetBoard()
{
  NUM_ROWS = Math.min(Math.max(4+level+GlassesChanges, 5), 13); 
  NUM_COLUMNS = NUM_ROWS;
  createBoard(); 
    ...
    poisonGhostCount = 0;
    if (level>4){
    NUM_GHOSTS = Math.min(Math.max(Math.floor(level/2+1), 1), 7);
  }else{
    NUM_GHOSTS = Math.min(Math.max(Math.floor(level/3+1), 1), 7)
  }
  ...
  if (level%3==1){
    ...
    // respawn pacman
    setTimeout( respawnPacmanTimer, RESET_PLAYER_DELAY*10);
...
function checkForPellets()
{
    ...
    if (!isOnItemLevel&&(pelletsEaten == totalPellets) && (numPowerPelletsEaten == POWER_PELLETS_START_COUNT))
    {
  ...
  switch(direction)
  {
    ...
    case ARROW_RIGHT:
      ...
      if ( (((current + 1) % NUM_COLUMNS) != 0) && (walls[current+1] == 0) )
      {
          ...
          return pacmanMovementChecks(PACMAN_CLASSIC_RIGHT,PACMAN_CLASSIC_RIGHT_PP);
      }
      break;
    ...
    case ARROW_LEFT:
      ...
      if ( ((current % NUM_COLUMNS) != 0) && (walls[current-1] == 0) )
      {
        ...
        return pacmanMovementChecks(PACMAN_CLASSIC_LEFT,PACMAN_CLASSIC_LEFT_PP);
      }
      break;
    ...
    case ARROW_DOWN:
      ...
      if ((current < NUM_COLUMNS * (NUM_ROWS-1)) && (walls[current+NUM_COLUMNS] == 0))
      {
        ...
        return pacmanMovementChecks(PACMAN_CLASSIC_DOWN,PACMAN_CLASSIC_DOWN_PP);
      }
      break;
    ...
    case ARROW_UP:
      ...
      {
        current = current - NUM_COLUMNS;
        return pacmanMovementChecks(PACMAN_CLASSIC_UP,PACMAN_CLASSIC_UP_PP);
      }
...
function pacmanMovementChecks(PACMANICON,PACMANICONPP)
{
  ...
  if(items[current]!=null&&isOnItemLevel){
    PickupItem(items[current],current)
  ...
  if (processPacmanTunnel())
    {
        pacmanIcon = (gameMode == GAME_MODE_POWER_ON) ? PACMANICONPP: PACMANICON;
        return SUCCESS;
    }
...
{
  for(i=0; i<ghosts.length,i++;){
    if(ghosts[i].pos==current){
      if(getRandomInt(0,100)<KnifeChance){
        eatGhosts();
      }
    }
  }
...
function spawnGhost(squareNum)
{
  ...
  { var tempGhostType = 0;
    ...
    var tempGhostType = getRandomInt(0,1);
  }else{
    var tempGhostType = getRandomInt(0,2);
  }
  
  // var tempGhostType = 2;

  if (tempGhostType == GHOST_TYPE_POISON && level>=6)
  {
    poisonGhostCount++;
